...
export const Modal = ({ children }: IModalProps) => {
  ...
  return (mountedModal && modalRef.current) ? createPortal((
    <div className={styles.modal}>
      <div className={styles.modalContent}>
        <div className={styles.closeBtnArea}>
          <button onClick={() => setMountedModal(!mountedModal)} className={styles.closeBtn}>
            <CloseIcon />
          </button>
        </div>
        <div className={styles.modalContentDisplay}>
          {children}
        </div>
      </div>
    </div>
