...
function ButtonText({ ...props }: IButtonProps) {
  return (
    <li className={`${styles.buttonDefault + ' ' + styles._Radiobody}`} >
      <input checked={props.defaultChecked} value={props.value} type="radio" id={props.id} name={props.name} className={styles.ButtonRadio} />
      <label htmlFor={props.id} className={styles.radioLabel}>{props.children}</label>
    </li>
  );
}
